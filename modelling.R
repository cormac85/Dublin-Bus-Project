...
medians <- vector()
#Next let's look at all days in the month for the same bus route.
for(i in 1:31){
...
# Verify vehicle journey ID's are unique #
query <- paste("SELECT * FROM table",2," WHERE LineID =", 67, sep='')
lineID.Data <- dbGetQuery(mydb, query)
namesDay2 <- names(table(lineID.Data$VehicleJourneyID))

query <- paste("SELECT * FROM table",3," WHERE LineID =", 67, sep='')
lineID.Data <- dbGetQuery(mydb, query)
namesDay3 <- names(table(lineID.Data$VehicleJourneyID))
